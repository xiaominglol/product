...
public class BusinessStoreClassificationController {
    ...
    @Autowired
    BusinessStoreClassificationService businessStoreClassificationService;
    ...
    public Message list(LayUiPage layUiPage, BusinessStoreClassificationPo businessStoreClassificationPo) {
        try {
            ...
            if (layUiPage.getPageNum() != 0 && layUiPage.getPageSize() != 0) {
                ...
                List<BusinessStoreClassificationPo> list = businessStoreClassificationService.list(qw);
                return Message.success(list);
            }
        } catch (Exception e) {
    ...
    public Message detail(@PathVariable("id") Long id) {
        try {
            if (!StringUtils.isEmpty(id)) {
                BusinessStoreClassificationPo businessStoreClassificationPo = businessStoreClassificationService.getById(id);
                return Message.success(businessStoreClassificationPo);
            } else {
    ...
    @PostMapping
    @ResponseBody
    public Message add(@RequestBody BusinessStoreClassificationPo po) {
        try {
            if (StringUtils.isEmpty(po.getId())) {
                businessStoreClassificationService.insertSync(po, po.getDetailList(), false);
    ...
    @PutMapping
    @ResponseBody
    public Message update(@RequestBody BusinessStoreClassificationPo po) {
        try {
            if (!StringUtils.isEmpty(po.getId())) {
                businessStoreClassificationService.updateSync(po, po.getDetailList(), false);
                return Message.success(po);
            } else {
                return Message.fail(CommonFailInfo.Id_CAN_NOT_BE_EMPTY);
            }
    ...
    public Message delete(@PathVariable("id") Long id) {
        try {
            if (!StringUtils.isEmpty(id)) {
                businessStoreClassificationService.deleteByIdSync(id);
