package com.gemini.business.member.service;

import com.gemini.boot.framework.mybatis.service.BaseDetailService;
import com.gemini.business.member.mapper.MemberCollectionMapper;
import com.gemini.business.member.po.MemberCollectionPo;

/**
 * 会员收藏表
 *
 * @author 小明不读书
 * @date Wed Dec 04 09:34:37 CST 2019
 */
public interface MemberCollectionService extends BaseDetailService<MemberCollectionPo, MemberCollectionPo, MemberCollectionMapper, MemberCollectionMapper> {
}